...
# saveRDS(seurat_object,here("data","processed", "single cell", "5_analysis", "annotated.rds"))
...
```{r Run, fig.width=40, fig.height=10}
...
nichenet_results$celltypes <- celltypes
...
nichenet_results$celltypes <- celltypes
saveRDS(nichenet_results, "NicheNet_hires_results.rds")
```

```{r}
nichenet_results <- readRDS(here("data", "processed", "single cell", "5_analysis", "NicheNet_hires_results.rds"))

# make names filename safe
names(nichenet_results)[3] <- "fibro:Dpp4+-Pi16+ (APC1+2)"
names(nichenet_results)[5] <- "adipo:mature adipo (AD1+2)"
names(nichenet_results)[10] <- "macro:unknown"
nichenet_results$celltypes <- names(nichenet_plots)[2:length(nichenet_plots)]

for (celltype in nichenet_results$celltypes) {
  savedir <- here("data", "processed", "single cell", "5_analysis", "nichenet", celltype)
  dir.create(savedir, showWarnings = FALSE)
  
  # agnostic plots
  ggsave(
    paste0(savedir, "/agnostic-","ligand_qc.jpg"),
    plot=nichenet_results[[celltype]]$agnostic.plots[[1]],
    width=5, height=3)
  ggsave(
    paste0(savedir, "/agnostic-","results.jpg"),
    plot=nichenet_results[[celltype]]$agnostic.plots[[2]],
    width=50, height=10, limitsize = FALSE)
  
  # sender-specific plots
  ggsave(
    paste0(savedir, "/sender-","ligand_qc.jpg"),
    plot=nichenet_results[[celltype]]$sender.plots[[1]],
    width=5, height=3)
  ggsave(
    paste0(savedir, "/sender-","results.jpg"),
    plot=nichenet_results[[celltype]]$sender.plots[[2]],
    width=50, height=10, limitsize = FALSE)
  ggsave(
    paste0(savedir, "/comparison.jpg"),
    plot=nichenet_results[[celltype]]$sender.plots[[3]],
    width=10, height=10)
}

print(nichenet_results$celltypes)

for (celltype in nichenet_results$celltypes) {
  nichenet_results[[celltype]]$agnostic.plots %>% print()
  nichenet_results[[celltype]]$sender.plots %>% print()
}
```

# Full Interactive NicheNet

## Run NicheNet

```{r}
cell.types <- as.factor(labels) %>% levels()
print(cell.types)
...
length(sender_potential_ligands)
```
...
# Find genes of interest. Can also take from AnnData object.
condition_oi <-  "HFD"
condition_reference <- "LFD"

# calculate DEGs
seurat_obj_receiver <- subset(seurat_object, subset = cell_type == receiver)
DE_table_receiver <-  FindMarkers(object = seurat_obj_receiver,
                                  ident.1 = condition_oi, ident.2 = condition_reference,
                                  group.by = "Condition",
                                  min.pct = 0.05) %>% rownames_to_column("gene")
...
```{r Plot top ligands, fig.width=6, fig.height=6}
best_upstream_ligands <- ligand_activities %>% top_n(n_ligands, aupr_corrected) %>% arrange(-aupr_corrected) %>% pull(test_ligand)
...
```{r Plot top targets, fig.width=20, fig.height=6}
active_ligand_target_links_df <- best_upstream_ligands %>%
  lapply(get_weighted_ligand_target_links,
         geneset = geneset_oi,
         ligand_target_matrix = ligand_target_matrix,
...
```{r Plot top receptors, fig.width=10, fig.height=6}
ligand_receptor_links_df <- get_weighted_ligand_receptor_links(
  best_upstream_ligands, expressed_receptors,
  lr_network, weighted_networks$lr_sig) 
...
```{r Plot top targets, fig.width=20, fig.height=5}
# Target gene plot
active_ligand_target_links_df <- best_upstream_ligands %>%
  lapply(get_weighted_ligand_target_links,
         geneset = geneset_oi,
...
```{r Find the senders, fig.width=12}
# Dotplot of sender-focused approach
p_dotplot <- DotPlot(subset(seurat_object, cell_type %in% senders),
        features = rev(best_upstream_ligands), cols = "RdYlBu") +
  coord_flip() +
  scale_y_discrete(position = "right")
...
## Compare Approaches
```{r Lineplot comparison, fig.height=10}
...
```{r Combined Results, fig.width=40, fig.height=10}
...
figures_without_legend <- cowplot::plot_grid(
  ...
  align = "hv",
  nrow = 1,
  rel_widths = c(ncol(vis_ligand_aupr)+6, ncol(vis_ligand_lfc)+7, ncol(vis_ligand_lfc)+8, nrow(vis_ligand_receptor_network), ncol(vis_ligand_target)))

ratio <- c(1.5, 2, 1.5, 6, 17)
legends <- cowplot::plot_grid(
    ggpubr::as_ggplot(ggpubr::get_legend(p_ligand_aupr)),
    ggpubr::as_ggplot(ggpubr::get_legend(p_dotplot)),
    ggpubr::as_ggplot(ggpubr::get_legend(p_lfc)),
